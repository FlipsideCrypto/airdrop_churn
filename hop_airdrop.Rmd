...
Looking at all HOP claimers on or before block 15890000 (Nov 3rd 2022) for reproducibility 
...
Getting claimer balances as of BLOCK_NUMBER = 15890000 to identify how quickly balances of recipients fell to < 1 (i.e., 0 or dust), focusing specifically on those that claimed >= 1 HOP (ignoring those that claimed dust amounts).
...
plot_ly(data = hop_month_churn,
        ...
        type = 'bar') %>% 
  layout(
    title = list(text = "Of the 60% that dumped, most did so immediately.",
                 y = 0.95),
...
plot_ly(alpha = 0.45,
  ...
  layout(
    ...
    yaxis = list(title = "# Claimers", range = c(0, 1500))
  )
...
has **more** HOP than they claimed; 
...
WHERE BLOCK_NUMBER <= 15890000 AND 
...
simple_out <- hop_transfers_by_claimers %>% 
  ...
  mutate(USER_ADDRESS = FROM_ADDRESS) %>% 
  group_by(USER_ADDRESS) %>% 
  summarise(simple_transfer_out = sum(AMOUNT)) 
...
simple_in <- hop_transfers_by_claimers %>%
  ...
  mutate(USER_ADDRESS = TO_ADDRESS) %>% 
  group_by(USER_ADDRESS) %>% 
  summarise(simple_transfer_in = sum(AMOUNT))
...
tbls = list(
  buys, sales, deposits, withdraws, simple_in, simple_out
)
...
- Dumpooooor: Claimed and sold everything (may have received from other EOA) w/o providing liquidity.
    - token_balance < 1
    - amount_bought = 0
...
- Airdrop Maximizer: Received tokens from another EOA and sold, never providing liquidity or buying.
    - amount_bought = 0
    - amount_sold > 0
    - simple_transfer_in > 0
    - simple_transfer_out >= 0
    - amount_deposited_lp = 0
...
- Intermediary: never bought, never provided liquidity; transferred equal or more than it claimed to another EOA. Possibly affiliated with an Airdrop Maximizer.
    - simple_transfer_out >= CLAIMED_TOKEN_VOLUME 
    - amount_bought == 0 
    - amount_deposited_lp == 0
...
class = case_when(
 TOKEN_BALANCE >= CLAIMED_TOKEN_VOLUME & amount_bought == 0 & amount_sold == 0 & amount_deposited_lp == 0 ~ "Keepooor",
 TOKEN_BALANCE < 1 & amount_bought == 0 & amount_sold >= CLAIMED_TOKEN_VOLUME & amount_deposited_lp == 0 ~ "Dumpooor",
 amount_deposited_lp > 0 & amount_withdrawn_lp >= 0 ~ "Market Maker",
 amount_bought > 0 & amount_sold > 0 & amount_deposited_lp == 0 ~ "Trader",
 amount_bought == 0 & amount_sold > 0 & simple_transfer_in > 0 &
   simple_transfer_out == 0 & amount_deposited_lp == 0 ~ "Airdrop Maximizer",
    simple_transfer_out >= CLAIMED_TOKEN_VOLUME & amount_bought == 0 & amount_deposited_lp == 0  ~ "Intermediary",
 TRUE ~ "N/A"
  )
)
